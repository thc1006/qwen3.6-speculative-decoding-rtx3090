...
win draft from inside the target — DFlash and the model's own
multi-token-prediction head — and the one that loses three quarters of the
...
generated tokens** — it is neutral because it almost never fires.
...
events at draft lengths 1–8 — comes from the ordinary verbose logs, a separate
evidence chain, and DFlash's 1–16 range does too. Re-applying the archived patch
to a stock tree rebuilt the instrumented library to the same `ce94855f…` byte
for byte, and reverting rebuilt to `a0cbe4d0…`.
...
`draft_n` is 2441 with acceptance 72.3 % in every single one — the speculative
...
explain nothing either — across the step the card is *hotter* (63.4 → 64.9 °C),
...
block, which confounds the mode with sixteen minutes of elapsed time — the third
review's P0-1, and correct. **Run V2 is the crossover it asked for.** Eight
sessions of two halves on 2026-08-27 in `AB BA BA AB BA AB AB BA` order, so each
...
`spec-dflash-n4` — the arm A17 was written about, published as "`n_max 4` goes
...
0.01–0.3 tok/s and the baseline to a hundredth, while `spec-dflash-n2` moves
...
> points the same way each time — that arm is slower after a capped neighbour —
...
- **Scope.** The controlled tier is runs A–V3, not A–T3. Its findings are for
  llama.cpp `3737e4137` under the recorded configuration, not "current
...
- **The headline interval** is within-invocation and is labelled so; the
  ...
  twelve independent measurements either — they mix builds, matrix sizes and
  neighbouring treatments — so `+17 % to +27 %` is a bound on what was observed,
  not a confidence interval, and the 43 nested blocks are not 43 replicates.
...
- **The request-mean columns.** `predicted_per_second` is llama.cpp's own field
  and it divides `n − 1` tokens by the time for `n` — 13 300 of 13 344 committed
  request rows, exactly. Every **request-mean** column in this repository
  inherits that, understating by `(n − 1) / n`: 0.33 % at 300 tokens. No
  headline figure or published delta contains it, because those are pooled rates
  computed from the two raw fields, and on a run where every request hits the
  same cap the bias is identical on every arm. Recomputing several dozen
